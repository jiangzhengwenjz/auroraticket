...
// struct RfuSlotStatusUNI *gUnk_03002520;
...
static u8 sRfuStaticLsFixedCount[RFU_CHILD_MAX];
static ALIGNED(8) u16 sRfuStaticCidBak[RFU_CHILD_MAX];
static u8 sRfuStaticSCStartFlag;
static ALIGNED(4) u8 sRfuStaticLinkEmergencyFlag[RFU_CHILD_MAX];
...
u16 rfu_initializeAPI(u32 *APIBuffer, u16 buffByteSize, IntrFunc *sioIntrTable_p, bool8 copyInterruptToRam)
{
    ...
    gRfuFixed = (void *)APIBuffer + 0xcc; // + sizeof(*gRfuStatic)
    gRfuSlotStatusNI[0] = (void *)APIBuffer + 0x1ac; // + sizeof(*gRfuFixed)
    gRfuSlotStatusUNI[0] = (void *)APIBuffer + 0x36c; // + sizeof(*gRfuSlotStatusNI[0]) * RFU_CHILD_MAX
    for (i = 1; i < RFU_CHILD_MAX; ++i)
    {
        gRfuSlotStatusNI[i] = &gRfuSlotStatusNI[i - 1][1];
        gRfuSlotStatusUNI[i] = &gRfuSlotStatusUNI[i - 1][1];
    }
    gUnk_03002520 = gRfuSlotStatusUNI[3] + 1;
    gRfuFixed->STWIBuffer = (struct RfuIntrStruct *)&gRfuSlotStatusUNI[3][3];
    STWI_init_all((struct RfuIntrStruct *)&gRfuSlotStatusUNI[3][3], sioIntrTable_p, copyInterruptToRam);
...
static void rfu_STC_clearAPIVariables(void)
{
    ...
    rfu_clearAllSlot();
    sRfuStaticSCStartFlag = 0;
    for (i = 0; i < RFU_CHILD_MAX; ++i)
        sRfuStaticCidBak[i] = 0;
    REG_IME = IMEBackup;
}
         ...
         data_p += 4)
    {
        ...
        if (bm_slot_id < RFU_CHILD_MAX && !((gRfuLinkStatus->connSlotFlag >> bm_slot_id) & 1) && !((gRfuLinkStatus->linkLossSlotFlag >> bm_slot_id) & 1))
        {
            if (true_slots[bm_slot_id] != 0)
                ++sRfuStaticLsFixedCount[bm_slot_id];
            if (sRfuStaticLsFixedCount[bm_slot_id] >= 4)
            {
                sRfuStaticLsFixedCount[bm_slot_id] = 0;
                ...
                sRfuStaticCidBak[bm_slot_id] = gRfuLinkStatus->partner[bm_slot_id].id;
            }
        }
        --numSlots;
    }
...
void rfu_REQ_endConnectParent(void)
{
    ...
    if (gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[6] < 4)
        sRfuStaticLinkEmergencyFlag[gRfuFixed->STWIBuffer->rxPacketAlloc.rfuPacket8.data[6]] = 0;
}
        ...
        if (reqResult == 0)
        {
            if (reasonMaybe == 1 && (gRfuLinkStatus->connSlotFlag & newLinkLossFlag))
            {
                if (gRfuLinkStatus->strength[i] == 0)
                {
                    if (gRfuLinkStatus->parentChild == MODE_PARENT)
                    {
                        ++sRfuStaticLinkEmergencyFlag[i];
                        if (sRfuStaticLinkEmergencyFlag[i] > 3)
                        {
                    ...
                    else
                    {
                        ...
                        if (STWI_poll_CommandEnd() == 0)
                        {
                            ...
                            else
                            {
                                if (++sRfuStaticLinkEmergencyFlag[i] > 3)
                                {
                                    sRfuStaticLinkEmergencyFlag[i] = 0;
                ...
                else
                {
                    if (!((gRfuLinkStatus->connSlotFlag | gRfuLinkStatus->linkLossSlotFlag) & newLinkLossFlag))
                    {
                        ...
                        for (packet_p += 8; num_packets != 0; packet_p += 4, --num_packets)
                        {
                            ...
                            if (packet_p[2] == i && cid == sRfuStaticCidBak[i])
                            {
